		...
		case WM_COMMAND:
		{
			...
			case static_cast<int>(WmMainWindowCommands::ButtonGenerate):
			{
				...
				case static_cast<int>(WmMainWindowCommands::ButtonFirstAndLastFrame):
				{
					...
					if (plugin->m_engine.m_additional_files_paths.size() != 2)
					{
						run = false;
						AVS::Label_SetTextAndColor(hStatus, CTranslate::GetInstance().GetManager()->Translate(L"Required input files. The first and last frames of the video."), AVS::Color::MakeRGBA(255, 0, 0));
						break;
					}
				...
				case static_cast<int>(WmMainWindowCommands::ButtonExtendVideo):
				{
					...
					{
						AVS::Label_SetTextAndColor(hStatus, CTranslate::GetInstance().GetManager()->Translate(L"The required input video file is generated by Veo3."), AVS::Color::MakeRGBA(255, 0, 0));
						break;
					};
