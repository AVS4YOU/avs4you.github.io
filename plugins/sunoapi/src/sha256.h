...
std::string CalcFileSHA256(const std::wstring &filePath)
{
	...
	status = BCryptGetProperty(hAlg, BCRYPT_HASH_LENGTH, reinterpret_cast<PUCHAR>(&cbHash), sizeof(DWORD), &cbData, 0);
	...
	while (file)
	{
		...
		if (bytesRead > 0)
		{
			status = BCryptHashData(hHash, reinterpret_cast<PUCHAR>(buffer.data()), static_cast<ULONG>(bytesRead), 0);
