...
public:
	ButtonInfo(const ButtonSettings &settings)
	{
		...
		hFont = CreateFontW(fontH, 0, 0, 0, Settings.FontBold ? FW_BOLD : FW_NORMAL, Settings.FontItalic ? TRUE : FALSE, FALSE, FALSE, DEFAULT_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS, DEFAULT_QUALITY, DEFAULT_PITCH | FF_SWISS, Settings.FontName.c_str());
	}
...
HWND CreateButton(HWND hParent, HMENU hMenu, HINSTANCE hInstance, LPCWSTR text, const int &x, const int &y, const int &w, const int &h, const ButtonSettings &settings)
{
	HWND hwnd = CreateWindow(L"BUTTON", text, WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON | BS_OWNERDRAW | WS_TABSTOP, x, y, w, h, hParent, hMenu, hInstance, NULL);
...
void Button_SetSettings(HWND hwnd, const ButtonSettings &settings, const std::wstring &text)
{
	...
	if (info)
	{
		...
		if (fontH > 0)
			fontH = -MulDiv(fontH, 96, 72);
		info->hFont = CreateFontW(fontH, 0, 0, 0, info->Settings.FontBold ? FW_BOLD : FW_NORMAL, info->Settings.FontItalic ? TRUE : FALSE, FALSE, FALSE, DEFAULT_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS, DEFAULT_QUALITY, DEFAULT_PITCH | FF_SWISS, info->Settings.FontName.c_str());
...
HWND CreateTextEditMultiline(HWND hParent, HINSTANCE hInstance, const int &x, const int &y, const int &w, const int &h, const TextEditSettings &settings)
{
	...
	HWND hwnd = CreateWindowEx(WS_EX_CLIENTEDGE, L"EDIT", L"", dwStyle, x, y, w, h, hParent, NULL, hInstance, NULL);
...
HWND CreateProgressBar(HWND hParent, HINSTANCE hInstance, int x, int y, int w, int h, const ProgressBarSettings &settings)
{
	HWND hwnd = CreateWindow(L"STATIC", L"", WS_CHILD | WS_VISIBLE, x, y, w, h, hParent, NULL, hInstance, NULL);
	...
	if (nCode >= 0 && g_OpenComboBoxInfo != nullptr)
	{
		if (wParam == WM_LBUTTONDOWN || wParam == WM_NCLBUTTONDOWN)
		{
			...
			if (pMouseStruct != NULL)
			{
				...
				if (hwndClicked != g_OpenComboBoxInfo->hComboBox && hwndClicked != g_OpenComboBoxInfo->hListBox)
				{
...
{
	if (g_OpenComboBoxInfo && g_OpenComboBoxInfo != info)
		CloseComboBoxDropdown(g_OpenComboBoxInfo);
	...
	if (!info->hListBox)
	{
		info->hListBox = CreateWindowEx(WS_EX_TOPMOST | WS_EX_TOOLWINDOW | WS_EX_NOACTIVATE, L"LISTBOX", NULL, WS_POPUP | LBS_NOTIFY | LBS_HASSTRINGS | WS_VSCROLL | WS_BORDER, rc.left, rc.bottom, rc.right - rc.left, dropdownHeight, hwnd, NULL, GetModuleHandle(NULL), NULL);
	...
	SetWindowPos(info->hListBox, HWND_TOPMOST, rc.left, rc.bottom, rc.right - rc.left, dropdownHeight, SWP_NOACTIVATE | SWP_SHOWWINDOW);
...
HWND CreateComboBox(HWND hParent, HMENU hMenu, HINSTANCE hInstance, int x, int y, int w, int h, const ComboBoxSettings &settings, const std::vector<std::wstring> &items)
{
	HWND hwnd = CreateWindow(L"STATIC", L"", WS_CHILD | WS_VISIBLE | WS_TABSTOP | SS_NOTIFY, x, y, w, h, hParent, hMenu, hInstance, NULL);
...
LRESULT CALLBACK _ComboBoxProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam, UINT_PTR uIdSubclass, DWORD_PTR dwRefData)
{
	...
	case WM_PAINT: {
		...
		if (info->SelectedIndex >= 0 && info->SelectedIndex < (int)info->Items.size())
		{
			...
			DrawText(hdc, info->Items[info->SelectedIndex].c_str(), -1, &textRect, DT_LEFT | DT_VCENTER | DT_SINGLELINE | DT_END_ELLIPSIS);
	...
	case WM_DRAWITEM: {
		...
		if (dis->hwndItem == info->hListBox && dis->CtlType == ODT_LISTBOX)
		{
			...
			if (isHovered)
			{
				bgColor = RGB(info->Settings.DropdownHoverColor.R, info->Settings.DropdownHoverColor.G, info->Settings.DropdownHoverColor.B);
				textColor = RGB(info->Settings.DropdownTextColor.R, info->Settings.DropdownTextColor.G, info->Settings.DropdownTextColor.B);
			}
			else
			{
				bgColor = RGB(info->Settings.DropdownBackground.R, info->Settings.DropdownBackground.G, info->Settings.DropdownBackground.B);
				textColor = RGB(info->Settings.DropdownTextColor.R, info->Settings.DropdownTextColor.G, info->Settings.DropdownTextColor.B);
			}
...
HWND CreateLabel(HWND hParent, HINSTANCE hInstance, LPCWSTR text, const int &x, const int &y, const int &w, const int &h, const LabelSettings &settings)
{
	HWND hwnd = CreateWindow(L"STATIC", text, WS_CHILD | WS_VISIBLE | SS_OWNERDRAW, x, y, w, h, hParent, NULL, hInstance, NULL);
