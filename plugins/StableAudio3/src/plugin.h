...
{
    std::wstring prompt, negativePrompt, model = L"medium", encoding = L"f16", device = L"cpu";
...
class CStableAudio3Plugin
{
  public:
    ...
    void Cancel();
    void JoinWorker();
    void PostStatus(const std::wstring &text, int progress = -1, bool finished = false, bool success = false);
};
