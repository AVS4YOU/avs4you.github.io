...
constexpr int CLIENT_HEIGHT = 560;
...
void CenterWindow(HWND hwnd)
{
    ...
    SetWindowPos(hwnd, nullptr, centerRect.left + (centerRect.right - centerRect.left - width) / 2,
                 centerRect.top + (centerRect.bottom - centerRect.top - height) / 2, 0, 0, SWP_NOZORDER | SWP_NOSIZE);
}
...
HWND CreateEdit(HWND parent, int x, int y, int width, int height, const std::wstring &value, bool multiline = false)
{
    AVS::TextEditSettings settings = AVS::TextEditSettings::Create();
    settings.IsMultiline = multiline;
    settings.IsVscroll = multiline;
    HWND edit = AVS::CreateTextEditMultiline(parent, GetModuleHandleW(nullptr), x, y, width, height, settings);
    SetWindowTextW(edit, value.c_str());
    return edit;
}

HWND CreateLabel(HWND parent, const wchar_t *text, int x, int y, int width, int height,
                 AVS::LabelType type = AVS::LabelType::Disabled, DWORD alignment = DT_LEFT | DT_VCENTER | DT_SINGLELINE)
{
    return AVS::CreateLabel(parent, GetModuleHandleW(nullptr), text, x, y, width, height,
                            AVS::LabelSettings::Create(type, alignment));
}

AVS::ButtonSettings CancelButtonSettings()
{
    AVS::ButtonSettings settings = AVS::ButtonSettings::Create(AVS::Buttons::Default);
...
void SetBusy(Controls *controls, bool busy)
{
    ...
    EnableWindow(controls->encoding, !busy);
    EnableWindow(controls->device, !busy);
    EnableWindow(controls->duration, !busy);
    AVS::Button_SetSettings(controls->action,
                            busy ? CancelButtonSettings() : AVS::ButtonSettings::Create(AVS::Buttons::Primary),
                            busy ? L"Cancel" : L"Generate");
}

void CloseSettings(HWND hwnd)
{
    auto *state = reinterpret_cast<SettingsWindow *>(GetWindowLongPtrW(hwnd, GWLP_USERDATA));
...
LRESULT CALLBACK SettingsWindowProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_CREATE: {
        ...
        const Field fields[] = {{L"Steps", 15, 110, &SettingsWindow::steps, std::to_wstring(values.steps)},
                                {L"Seed", 140, 120, &SettingsWindow::seed, std::to_wstring(values.seed)},
                                {L"CFG scale", 275, 110, &SettingsWindow::cfg, std::to_wstring(values.cfg)},
                                {L"CPU threads", 400, 110, &SettingsWindow::threads, std::to_wstring(values.threads)},
                                {L"Padding, sec", 525, 110, &SettingsWindow::padding, std::to_wstring(values.padding)}};
        for (const auto &field : fields)
        {
            CreateLabel(hwnd, field.label, field.x, 15, field.width, 20);
            state->*(field.member) = CreateEdit(hwnd, field.x, 38, field.width, 28, field.value);
        }
        ...
        state->shift = AVS::CreateComboBox(hwnd, reinterpret_cast<HMENU>(static_cast<INT_PTR>(204)), instance, 15, 104,
                                           180, 28, comboSettings, shifts);

        state->keepModels = CreateWindowExW(
            0, L"BUTTON", L"Keep models loaded in memory", WS_CHILD | WS_VISIBLE | WS_TABSTOP | BS_AUTOCHECKBOX, 220,
            105, 255, 26, hwnd, reinterpret_cast<HMENU>(static_cast<INT_PTR>(ID_SETTINGS_KEEP)), instance, nullptr);
        SendMessageW(state->keepModels, WM_SETFONT, reinterpret_cast<WPARAM>(GetStockObject(DEFAULT_GUI_FONT)), TRUE);
        SendMessageW(state->keepModels, BM_SETCHECK, values.keepModels ? BST_CHECKED : BST_UNCHECKED, 0);

        AVS::CreateButton(hwnd, reinterpret_cast<HMENU>(static_cast<INT_PTR>(ID_SETTINGS_CANCEL)), instance, L"Cancel",
                          425, 157, 100, 30, AVS::ButtonSettings::Create(AVS::Buttons::Default));
        AVS::CreateButton(hwnd, reinterpret_cast<HMENU>(static_cast<INT_PTR>(ID_SETTINGS_SAVE)), instance, L"Save", 535,
                          157, 100, 30, AVS::ButtonSettings::Create(AVS::Buttons::Primary));
        return 0;
    }
    case WM_COMMAND:
        if (LOWORD(wParam) == ID_SETTINGS_SAVE && state && state->controls)
        {
            ...
            values.keepModels = SendMessageW(state->keepModels, BM_GETCHECK, 0, 0) == BST_CHECKED;
            CloseSettings(hwnd);
            return 0;
        }
...
void ShowSettingsWindow(HWND owner, Controls *controls)
{
    ...
    HWND window = CreateWindowExW(0, className, L"Stable Audio 3 Settings", style, CW_USEDEFAULT, CW_USEDEFAULT,
...
{
    auto *plugin = reinterpret_cast<CStableAudio3Plugin *>(GetWindowLongPtrW(hwnd, GWLP_USERDATA));
    auto *controls = reinterpret_cast<Controls *>(GetPropW(hwnd, L"StableAudio3.Controls"));
    ...
    {
    case WM_CREATE: {
        plugin = reinterpret_cast<CStableAudio3Plugin *>(GetWindowLongPtrW(hwnd, GWLP_USERDATA));
        ...
        controls->model =
            AVS::CreateComboBox(hwnd, reinterpret_cast<HMENU>(static_cast<INT_PTR>(ID_MODEL)), instance, 15, 267, 190,
                                28, AVS::ComboBoxSettings::Create(), {L"medium", L"small-music", L"small-sfx"});
        controls->duration = CreateEdit(hwnd, 220, 267, 100, 28, L"12.0");
        controls->download =
            AVS::CreateButton(hwnd, reinterpret_cast<HMENU>(static_cast<INT_PTR>(ID_DOWNLOAD)), instance,
                              L"Download model", 565, 267, 180, 28, AVS::ButtonSettings::Create(AVS::Buttons::Default));

        CreateLabel(hwnd, L"Encoding", 15, 311, 105, 20);
        CreateLabel(hwnd, L"Device", 135, 311, 95, 20);
        controls->encoding =
            AVS::CreateComboBox(hwnd, reinterpret_cast<HMENU>(static_cast<INT_PTR>(ID_ENCODING)), instance, 15, 333,
                                105, 28, AVS::ComboBoxSettings::Create(), {L"f16", L"f32"});
        controls->device = AVS::CreateComboBox(hwnd, reinterpret_cast<HMENU>(static_cast<INT_PTR>(ID_DEVICE)), instance,
                                               135, 333, 95, 28, AVS::ComboBoxSettings::Create(), {L"cpu", L"gpu"});

        CreateLabel(hwnd, L"System resources and available acceleration", 15, 379, 730, 22);
        CreateLabel(hwnd, GetSystemSummary(plugin->workDirectory).c_str(), 15, 403, 730, 82, AVS::LabelType::Enabled,
                    DT_LEFT | DT_TOP | DT_WORDBREAK);
        controls->progress =
            AVS::CreateProgressBar(hwnd, instance, 15, 489, 730, 22, AVS::ProgressBarSettings::Create());
        AVS::ProgressBar_SetRange(controls->progress, 0, 100);
        controls->status =
            CreateLabel(hwnd, L"Ready", 15, 522, 480, 28, AVS::LabelType::Enabled, DT_LEFT | DT_TOP | DT_WORDBREAK);
        controls->settingsButton =
            AVS::CreateButton(hwnd, reinterpret_cast<HMENU>(static_cast<INT_PTR>(ID_SETTINGS)), instance, L"Settings",
                              515, 520, 105, 30, AVS::ButtonSettings::Create(AVS::Buttons::Default));
        controls->action =
            AVS::CreateButton(hwnd, reinterpret_cast<HMENU>(static_cast<INT_PTR>(ID_ACTION)), instance, L"Generate",
                              635, 520, 110, 30, AVS::ButtonSettings::Create(AVS::Buttons::Primary));
    ...
    case WM_COMMAND:
        ...
        case ID_ACTION:
            ...
            else
            {
                ...
                {
                    AVS::Label_SetText(controls->status, L"Enter a prompt before generation.");
                    return 0;
                }
                ...
                {
                    AVS::Label_SetText(controls->status, L"Duration must be greater than zero.");
                    return 0;
                }
                const float maximumDuration = options.model == L"medium" ? 380.0f : 120.0f;
                if (durationSeconds > maximumDuration)
                {
                    AVS::Label_SetText(controls->status, L"Maximum duration for " + options.model + L" is " +
                ...
                options.frames = (std::max)(1, static_cast<int>(std::ceil(static_cast<double>(durationSeconds) *
                ...
                if (!IsModelSetPresent(plugin->workDirectory / L"models", options.model, options.encoding))
                {
                    AVS::Label_SetText(controls->status, L"Model files are missing. Click Download model first.");
                    return 0;
                }
                AVS::ProgressBar_SetPos(controls->progress, 0);
                SetBusy(controls, true);
    ...
    case WM_SA3_STATUS:
        if (plugin && controls)
        {
            ...
            if (status->finished)
            {
                ...
                {
                    plugin->callback(export_str(L"StableAudio3.plugin"), export_str(plugin->lastOutput.c_str()), 0,
...
void ShowStableAudioWindow(CStableAudio3Plugin *plugin)
{
    ...
    const bool activationActive =
        activationContext != INVALID_HANDLE_VALUE && ActivateActCtx(activationContext, &activationCookie);
    ...
    if (hwnd)
    {
        ...
        while (GetMessageW(&message, nullptr, 0, 0) > 0)
        {
            ...
            if (settings && IsWindow(settings) && IsDialogMessageW(settings, &message))
                continue;
