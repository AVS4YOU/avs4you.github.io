...
#ifdef _WIN32
    const int count = MultiByteToWideChar(CP_UTF8, 0, onnx_path.c_str(), -1, nullptr, 0);
    std::wstring wide_path(static_cast<size_t>(count - 1), L'\0');
    MultiByteToWideChar(CP_UTF8, 0, onnx_path.c_str(), -1, wide_path.data(), count);
